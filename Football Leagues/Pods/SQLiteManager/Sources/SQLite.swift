...
open class SQLite {
	...
	fileprivate var readConnection: OpaquePointer? // is used for reading
	fileprivate var writeConnection: OpaquePointer? // is used for writting
	fileprivate var batchConnection: OpaquePointer? // is used for batch processing
	...
	internal func initialize(database name: String, withExtension: String, createIfNotExist create: Bool = false) throws -> SQLite {
		...
		if (!moved) {
			...
			do { moved = try copyDatabaseFromBundleToDocumentsDir() } catch let e as NSError {
                throw e
            }
		} else {
...
public extension SQLite {
	...
	fileprivate func executeSQL(_ sqlStrings: [String]) throws -> SQLiteBatchQueryResult {
		...
		_ = self.closeClosure(batchConnection!, statement!)
...
fileprivate extension SQLite {
    ...
    fileprivate func createDatabaseFileAtPath(_ path: String) -> Bool {
        ...
        let fileManager = FileManager.default
        let created =  fileManager.createFile(atPath: path, contents: nil, attributes: [FileAttributeKey(rawValue: FileAttributeKey.creationDate.rawValue): Date(), FileAttributeKey(rawValue: FileAttributeKey.type.rawValue): FileAttributeType.typeRegular])
